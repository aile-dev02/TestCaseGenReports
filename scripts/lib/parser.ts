...
export function parseTestSpec(content: string, filePath: string): ParsedTestSpec {
  ...
  for (const line of lines) {
    ...
    if (cells.length >= 5 && /^TC-/.test(cells[0])) {
      const jaStatusMap: Record<string, string> = {
        '質問待ち': 'WAITING',
        '未実行': 'NOT_EXECUTED',
        '合格': 'PASS',
        '失敗': 'FAIL',
      ...
      const rawCell = cells[10]?.trim() ?? ''
      const rawStatus = jaStatusMap[rawCell] ?? rawCell.toUpperCase()
      const validStatuses = ['PASS', 'FAIL', 'SKIP', 'NOT_EXECUTED', 'NA', 'WAITING'] as const
      testCases.push({
        id: cells[0],
        テスト名: cells[1],
        種別: cells[2],
        優先度: cells[3] || undefined,
        手順: cells[4],
        期待結果: cells[5],
        上流ID: cells[6] || undefined,
        備考: cells[7] || undefined,
        担当者: cells[8] || undefined,
        完了日時: cells[9] || undefined,
